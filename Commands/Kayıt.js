...
module.exports = {
    ...
    message.reply({ embeds: [embed.setDescription(`🚫 Üzgünüm, ${member}. Sunucumuz şu an **Taglı Alım**dadır. Sunucumuza kayıt olmak için ${config.Other.Tag.map((x) => x).join(", ")} taglarından en az birini almalı, **boost** basmalı ya da **VIP** üye olmalısın.`)] }).catch((error) => console.log(error)).then((x) => setTimeout(() => { x.delete(); }, 10000));
        }
      }
    ...
    name = `${config.Other.NamePrefix ? config.Other.NamePrefix : ""} ${name} ${config.Other.NameSymbol ? config.Other.NameSymbol : "'"} ${age}`;
} else if (config.Modes.Name == "false") {
...
collector.on('collect', async (button) => {
    if (button.isButton()) {
        if (button.customId === "erkek") {
            ...
            await names.findOneAndUpdate({ guildID: message.guild.id, userID: member.user.id }, { $push: { isimler: { name: name, yetkili: message.author.id,  rol: config.Roles.Man[0], date: Date.now() } } }, { upsert: true });
            ...
            await names.findOneAndUpdate({ guildID: message.guild.id, userID: member.user.id }, { $push: { isimler: { name: name, yetkili: message.author.id,  rol: config.Roles.Woman[0], date: Date.now() } } }, { upsert: true });
